...
function Reclamations() {
  ...
  useEffect(() => {
    ...
    let apiUrl = "http://localhost:8005/api/troc/reclamation/post";

    if (status !== "all") {
      apiUrl = `http://localhost:8005/api/troc/reclamation/post/byStatus?status=${status}`;
    } else if (searchTitle) {
      apiUrl = `http://localhost:8005/api/troc/reclamation/post/search?title=${searchTitle}`;
    }
